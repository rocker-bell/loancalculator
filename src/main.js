...
class LoanCalculator {
    ...
    init() {
        document.querySelector('button').addEventListener('click', () => this.calculate());
    }

    calculate() {
        const amount = parseFloat(document.getElementById('amount').value) || 10000;
        const annualRate = parseFloat(document.getElementById('annual').value) || 5;
        const years = parseFloat(document.getElementById('years').value) || 30;

        const monthlyRate = annualRate / 1200;
        const totalPayments = years * 12;
        const monthlyPayment = this.calculateMonthlyPayment(amount, monthlyRate, totalPayments);

        if (isFinite(monthlyPayment)) {
            this.updateUI(monthlyPayment, amount, totalPayments);
            this.drawGraph(amount, monthlyRate, totalPayments, monthlyPayment);
        }
    }

    calculateMonthlyPayment(principal, monthlyRate, totalPayments) {
        return (principal * monthlyRate) / (1 - Math.pow(1 + monthlyRate, -totalPayments));
    }

    updateUI(monthlyPayment, principal, totalPayments) {
    ...
    drawGraph(principal, monthlyRate, totalPayments, monthlyPayment) {
        this.ctx.clearRect(0, 0, this.graph.width, this.graph.height);
        
        const getX = (payment) => (payment * this.graph.width / totalPayments);
        const getY = (amount) => {
            const maxAmount = monthlyPayment * totalPayments * 1.05;
            return this.graph.height - (amount * this.graph.height / maxAmount);
        };

        // Draw axes
        this.ctx.beginPath();
        this.ctx.moveTo(0, this.graph.height - 1);
        this.ctx.lineTo(this.graph.width, this.graph.height - 1);
        this.ctx.lineTo(this.graph.width - 1, 0);
        this.ctx.stroke();
        ...
        this.ctx.beginPath();
        this.ctx.moveTo(0, getY(balance));

        for (let payment = 1; payment <= totalPayments; payment++) {
            const interest = balance * monthlyRate;
            equity += (monthlyPayment - interest);
            balance = principal - equity;
            this.ctx.lineTo(getX(payment), getY(balance));
        }
        this.ctx.stroke();
    }
}
